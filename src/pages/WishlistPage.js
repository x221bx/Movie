...
import { selectWishlist, removeWish } from "../store/wishlistSlice";
import { Star } from "lucide-react";

export default function WishlistPage() {
    const items = useSelector(selectWishlist);
    const dispatch = useDispatch();
    if (items.length === 0) return <div className="text-center mt-5">Wish list is empty.</div>;
    return (
                    ...
                    <Col key={m.id} xs={6} md={4} lg={3} className="mb-4">
                        <Card className="h-100 bg-dark text-white">
                            ...
                            <Card.Body className="d-flex flex-column">
                                ...
                                <Button onClick={() => dispatch(removeWish(m.id))} variant="outline-light" className="mt-auto">Remove</Button>
                            </Card.Body>
                        </Card>
                    </Col>
                ))}
